...
fun QuestionsLeadersheapScreen(
    ...
    LaunchedEffect(userAssessmentsState) {
        if (userAssessmentsState is UserAssessmentsUiState.Loaded) {
            val hasAnswered = userAssessmentsViewModel.hasAnsweredCategory("Liderança")
            if (hasAnswered) {
                Toast.makeText(
                    context,
    ...
    val remoteQuestions: List<WorkloadQuestion>? = when (uiState) {
        is QuestionsUiState.Loaded -> {
            ...
            val filtered = loaded.filter { it.category.equals("Liderança", ignoreCase = true) }
    ...
    val handleFinished = { answers: List<String?> ->
        ...
        if (userId != null && ui is QuestionsUiState.Loaded) {
            val apiMap = ui.questions.filter { it.category.equals("Liderança", true) }
